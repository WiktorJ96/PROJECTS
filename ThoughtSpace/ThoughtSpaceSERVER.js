...
class NoteStorage {
  ...
  static async deleteAllNotes() {
    try {
      ...
      if (!response.ok) {
        throw new NoteError(
          "Nie udało się usunąć wszystkich notatek",
          "DELETE_ALL_NOTES_ERROR",
        );
      }
    } catch (error) {
...
class NoteApp {
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener("click", () => this.deleteAllNotes());
    ...
    document
      .getElementById("confirm-all-yes")
      .addEventListener("click", () => this.confirmDeleteAll());
    document
      .getElementById("confirm-all-no")
      .addEventListener("click", () => this.cancelDeleteAll());
  ...
  installPWA() {
    if (this.deferredPrompt) {
      ...
      alert(
        'Aby zainstalować aplikację, użyj opcji "Dodaj do ekranu głównego" w menu udostępniania przeglądarki.',
      );
    }
  }
      ...
      if (note) {
        ...
        colorOptions.forEach((option) => {
          option.classList.toggle(
            "selected",
            option.getAttribute("data-color") === this.selectedColor,
          );
        });
  ...
  searchNotes(searchTerm) {
    ...
    notes.forEach((note) => {
      ...
      note.style.display =
        title.includes(lowerSearchTerm) || body.includes(lowerSearchTerm)
          ? "block"
          : "none";
    });
  }
  ...
  static setNoteColor(note, color) {
    ...
    note.style.backgroundImage =
      gradients[color] || "linear-gradient(135deg, #ffffff, #f0f0f0)";
  }
}
